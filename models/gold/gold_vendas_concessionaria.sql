...
FROM {{ source('sources', 'silver_vendas') }} v
JOIN {{ source('sources', 'silver_concessionarias') }} con ON v.concessionaria_id = con.concessionaria_id
JOIN {{ source('sources', 'silver_cidades') }} cid ON con.cidade_id = cid.cidade_id
JOIN {{ source('sources', 'silver_estados') }} est ON cid.estado_id = est.estado_id
