...
JOIN {{ source('sources', 'silver_vendedores') }} ven ON v.vendedor_id = ven.vendedor_id
JOIN {{ source('sources', 'silver_concessionarias') }} c ON c.concessionaria_id = ven.concessionaria_id
